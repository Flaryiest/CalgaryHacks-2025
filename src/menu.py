...
class MenuSecondary:
    ...
    def drawMenuOptions(self, screen, menu_x, screen_width, screen_height):
        ...
        restart_button = pygame.transform.scale(
            pygame.image.load("assets/menu assets/Green restart button, Unpressed.png"),
            (30, 30),
        )
        ...
        slider_height = 10
        pygame.draw.rect(
            screen, (255, 255, 255), (slider_x, slider_y, slider_width, slider_height)
        )
    ...
    def handleMouseClick(self, pos, screen):
        ...
        if (
            button_x <= pos[0] <= button_x + button_width
            and restart_button_y <= pos[1] <= restart_button_y + button_height + 20
        ):
        ...
        if (
            ...
            and slider_y <= pos[1] <= slider_y + slider_height
        ):
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if menu_icon_notpressed.get_rect(topleft=(20, 20)).collidepoint(
                        event.pos
                    ) or menu_icon_pressed.get_rect(topleft=(20, 20)).collidepoint(
                        event.pos
                    ):
            ...
            if self.menu_open:
                self.drawMenu(
                    screen, self.menu_x, background_image, screen_width, screen_height
                )
            else:
                self.drawMenu(
                    screen, self.menu_x, background_image, screen_width, screen_height
